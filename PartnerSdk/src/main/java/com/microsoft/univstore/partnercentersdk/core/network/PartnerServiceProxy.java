package com.microsoft.univstore.partnercentersdk.core.network;
...
import com.microsoft.univstore.partnercentersdk.core.errorhandling.IFailedPartnerServiceResponseHandler;
...
import com.microsoft.univstore.partnercentersdk.core.exception.PartnerResponseParseException;
...
import com.microsoft.univstore.partnercentersdk.core.requestcontext.RequestContext;
import com.microsoft.univstore.partnercentersdk.core.requestcontext.RequestContextFactory;
import com.microsoft.univstore.partnercentersdk.core.utils.InvoiceLineItemDeserializer;
...
// -----------------------------------------------------------------------
// <copyright file="PartnerServiceProxy.cs" company="Microsoft">
 ...
 * deserializes the response from JSON into the given response type. The type of content that will be passed to the
 * partner service API.The type of response produced from the partner service API.
 */
public class PartnerServiceProxy<TRequest, TResponse>
    ...
    implements IPartnerServiceProxy<TRequest, TResponse>
{
     ...
     */
    public PartnerServiceProxy( TypeReference<TResponse> responseClass, IPartnerOperations rootPartnerOperations,
                                String resourcePath )
    {
     ...
     * @param jsonConverter An optional JSON response converter. The default will be used if not provided.
     */
    public PartnerServiceProxy( IPartnerOperations rootPartnerOperations, String resourcePath,
                                IFailedPartnerServiceResponseHandler errorHandler, ObjectMapper jsonConverter )
    {
        ...
        if ( this.getPartner().getRequestContext().getRequestId().equals( new UUID( 0, 0 ) ) )
        {
            ...
            this.requestContext =
                RequestContextFactory.create( this.getPartner().getRequestContext().getCorrelationId(),
    ...
    {
        this.requestContext = RequestContextFactory.create( this.requestContext.getCorrelationId(),
    ...
    {
        this.requestContext = RequestContextFactory.create( value, this.requestContext.getRequestId() );
    }
     ...
     */
    private Collection<KeyValuePair<String, String>> __AdditionalRequestHeaders =
    ...
    {
        request.setHeader( "MS-Contract-Version", MicrosoftPartnerSdk.getPartnerServiceApiVersion() );
        ...
        if( MicrosoftPartnerSdk.getPartnerServiceApiVersion() != null && MicrosoftPartnerSdk.getPartnerServiceApiVersion().trim().isEmpty() != true )
        {
        	request.setHeader( "MS-PartnerCenter-Application", MicrosoftPartnerSdk.getApplicationName() );
        }
    ...
    private void validateCredentials()
    {
        ...
        {
            if ( MicrosoftPartnerSdk.getRefreshCredentialsHandler() != null )
            {
                try
                {
                    MicrosoftPartnerSdk.getRefreshCredentialsHandler().onCredentialsRefreshNeeded( this.getPartner().getCredentials(),
                                                                                                   this.requestContext );
                }
    ...
    private TResponse handleResponse( CloseableHttpResponse response )
    {
        if ( response.getStatusLine().getStatusCode() < 400 )
        {
            ...
            try
            {
                ...
                TResponse responseObj = getJsonConverter().readValue( responseBody, responseClass );
                response.close();
                return responseObj;
            }
     ...
     * 
     * @return The complete partner service API Uri needed to invoke the current operation.
     */
    private URI buildPartnerServiceApiUri()
    {
        StringBuilder baseUri = new StringBuilder(MicrosoftPartnerSdk.getPartnerServiceApiRoot() + "/"
                        + MicrosoftPartnerSdk.getPartnerServiceApiVersion() + "/" + this.getResourcePath());
